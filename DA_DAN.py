...
class Encoder_E(nn.Module):
    ...
    def forward(self, enc_inputs,for_e):
        ...
        # return enc_outputs.view(enc_outputs.size(0),-1)
...
class DADAN(nn.Module):
    def __init__(self,en_feature_in,common_dim,de_feature_out,num_class):
        ...
        '''
        A is initialized to the sequence level of the source domain, indicating the extractor, 
        B is initialized to the sequence level of the target domain, indicating the extractor.
        Extract the sequence level representation from two different distributions respectively
        ...
        self.domainClassifier_A = nn.Sequential(
            # A gradient inversion layer is inserted into the first layer of the discriminator to realize confrontation learning
            GRL_Layer(),                                                       
        ...
        # self.domainClassifier_A=DomainClassifier()
        # self.domainClassifier_B=DomainClassifier()
        # taskClassifier Used to train a rater on the source domain
    ...
    def forward(self,seq_A,seq_B):
        ...
        # hinder_vertor_B=self.encoder_B(seq_B)
        recon_A = self.decoder_common(hinder_vertor_A)
        # recon_B=self.decoder_B(hinder_vertor_B)
        recon_B = self.decoder_common(hinder_vertor_B)
        common_A = self.project_common(hinder_vertor_A)
        common_B = self.project_common(hinder_vertor_B)
        # common_B=self.project_B(hinder_vertor_B)
    ...
    def predict(self,seq_B):
        ...
        seq_B = self.seuence_extract_B(seq_B)
        # seq_A=self.seuence_extract_A(seq_A,seq_B)
        # hinder_vertor_A=self.encoder_A(seq_A)
        hinder_vertor_B = self.encoder_common(seq_B)
        # hinder_vertor_B=self.encoder_B(seq_B)
        # common_A=self.project_A(hinder_vertor_A)
        common_B = self.project_common(hinder_vertor_B)
        # common_B=self.project_B(hinder_vertor_B)
        # classifer_A=self.taskClassifier(common_A)
